...
const CheckoutPage = observer(() => {
  ...
  const checkoutType =
    (location.state as { checkoutType?: "samples" | "table" } | null)
      ?.checkoutType ?? "table";
  const isSampleCheckout = checkoutType === "samples";
  ...
  const sampleItems = useMemo(() => {
    if (!isSampleCheckout) return [] as { name: string; previewUrl: string }[];

    const stateItems = (
      location.state as
        | { sampleItems?: { name: string; previewUrl: string }[] }
        | null
    )?.sampleItems;
    if (stateItems?.length) return stateItems;

    const stateNames = (
      location.state as { sampleNames?: string[] } | null
    )?.sampleNames;
    if (stateNames?.length) {
      return stateNames.map((name) => ({ name, previewUrl: "" }));
    }

    try {
      const raw = localStorage.getItem(ORDER_SAMPLES_STORAGE_KEY);
      if (!raw) return [] as { name: string; previewUrl: string }[];
      const parsed = JSON.parse(raw) as {
        names?: string[];
        samples?: { name: string; previewUrl: string }[];
      };
      if (parsed.samples?.length) return parsed.samples;
      return (parsed.names ?? []).map((name) => ({ name, previewUrl: "" }));
    } catch {
      return [] as { name: string; previewUrl: string }[];
    }
  }, [isSampleCheckout, location.state]);
  const sampleNames = sampleItems.map((item) => item.name);
        ...
        <div>
          <div className="mb-8">
            <h1 className="text-3xl font-semibold tracking-tight text-[#111]">
              {isSampleCheckout ? "Sample Checkout" : "Checkout"}
            </h1>
            ...
            <div className="mt-10 flex gap-4 rounded-lg bg-[#f5f5f5] p-5 border border-gray-200">
              ...
              <div>
                ...
                <p className="mt-1 text-xs leading-relaxed text-gray-600">
                  ...
                  make any changes before production process begins. After this
                  point, cancellations and amendments will not be possible.
                </p>
              </div>
            </div>
          </form>
